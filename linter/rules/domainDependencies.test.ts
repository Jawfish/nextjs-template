...
describe('domain-dependencies rule', () => {
  ...
  test('allows self-imports within a domain', async () => {
    const code = `import { Task } from '@/lib/domain/task/types';`;
    const errors = await lint('src/lib/domain/task/repository.ts', code);
  ...
  test('allows imports from domain root files like cache', async () => {
    const code = `import { withCache } from '@/lib/domain/cache';`;
    const errors = await lint('src/lib/domain/task/cached.ts', code);
    expect(errors).toHaveLength(0);
  });

  test('allows configured domain imports', async () => {
    const code = `
      import { createTaskRepository } from '@/lib/domain/task/repository';
      import { createStepRepository } from '@/lib/domain/step/repository';
    `;
    const errors = await lint('src/lib/domain/import/processor.ts', code);
    expect(errors).toHaveLength(0);
  });

  test('blocks unconfigured domain imports', async () => {
    const code = `import { createImportRepository } from '@/lib/domain/import/repository';`;
    const errors = await lint('src/lib/domain/task/repository.ts', code);
  ...
  test('blocks imports between unrelated leaf domains', async () => {
    const code = `import { Handler } from '@/lib/domain/handler/types';`;
    const errors = await lint('src/lib/domain/project/service.ts', code);
    expect(errors).toHaveLength(1);
    expect(errors[0].message).toContain(
  ...
  test('blocks leaf domains from importing anything', async () => {
    const code = `import { Step } from '@/lib/domain/step/types';`;
    const errors = await lint('src/lib/domain/handler/logic.ts', code);
    expect(errors).toHaveLength(1);
  ...
  test('allows wildcard domain imports for integration domain', async () => {
    const code = `
      import { createTaskRepository } from '@/lib/domain/task/repository';
      import { createStepRepository } from '@/lib/domain/step/repository';
      import { createImportRepository } from '@/lib/domain/import/repository';
    `;
    const errors = await lint('src/lib/domain/integration/helpers.ts', code);
    expect(errors).toHaveLength(0);
  });
  ...
  test('skips test files entirely', async () => {
    const code = `
      ...
      import { createImportRepository } from '@/lib/domain/import/repository';
    `;
    const errors = await lint('src/lib/domain/handler/repository.test.ts', code);
    expect(errors).toHaveLength(0);
  });
  ...
  test('hook can import from task (including state submodule)', async () => {
    const code = `import { StateEvent } from '@/lib/domain/task/state/types';`;
    const errors = await lint('src/lib/domain/hook/logic.ts', code);
    expect(errors).toHaveLength(0);
  });

  test('task/state submodule is part of task domain', async () => {
    const code = `import { Task } from '@/lib/domain/task/types';`;
    const errors = await lint('src/lib/domain/task/state/service.ts', code);
    expect(errors).toHaveLength(0);
  });

  test('task can import from step (including display-property submodule)', async () => {
    const code = `import { DisplayProperty } from '@/lib/domain/step/display-property/types';`;
    const errors = await lint('src/lib/domain/task/repository.ts', code);
    expect(errors).toHaveLength(0);
  });
  ...
  test('step/display-property submodule is part of step domain', async () => {
    ...
    const errors = await lint(
      'src/lib/domain/step/display-property/repository.ts',
      code
    );
    expect(errors).toHaveLength(0);
  });
  ...
  test('fails closed for unknown domains', async () => {
    const code = `import { something } from '@/lib/domain/task/types';`;
    const errors = await lint('src/lib/domain/unknown-domain/file.ts', code);
    expect(errors).toHaveLength(1);
    expect(errors[0].message).toContain(
